...
class DbHelperFa3 extends SQLiteOpenHelper {
    ...
    public DbHelperFa3(@Nullable Context context) {
    ...
    public long addTOFav(com.example.myapplication.NewArrival newArrival) {
    ...
    public int deleteFromCart(com.example.myapplication.NewArrival newArrival){

        SQLiteDatabase db = getWritableDatabase();
        return db.delete(TABLE_Favorite, KEY_CART_PRODUCT_ID + "=" +newArrival.getProductId(), null);

    }


    public boolean isWallpaperFavorite(com.example.myapplication.NewArrival newArrival) {
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery("select * from " + TABLE_Favorite + " where " + KEY_CART_PRODUCT_ID + "=" + newArrival.getProductId(),null);
    ...
    @SuppressLint("Range")
    public ArrayList<com.example.myapplication.NewArrival> getAllCartItems(){
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM "+ TABLE_Favorite, null);
        ArrayList<com.example.myapplication.NewArrival> cartItemList = new ArrayList<>();
