...
public class DbHelperFav extends SQLiteOpenHelper {
    ...
    public DbHelperFav(@Nullable Context context) {
    ...
    public long addTOFav(Product product) {
    ...
    public int deleteFromCart(Product product){
        ...
        SQLiteDatabase db = getWritableDatabase();
        return db.delete(TABLE_Favorite, KEY_CART_PRODUCT_ID + "=" +product.getProductId(), null);

    }


        public boolean isWallpaperFavorite(Product product) {
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery("select * from " + TABLE_Favorite + " where " + KEY_CART_PRODUCT_ID + "=" + product.getProductId(),null);
    ...
    public ArrayList<Product> getAllCartItems(){
        ...
        ArrayList<Product> cartItemList = new ArrayList<>();
