...
public class DbHelper extends SQLiteOpenHelper {
    ...
    public boolean isProductInCart(Product product) {
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT "+ KEY_CART_ID +" FROM " + TABLE_CART + " WHERE " + KEY_CART_PRODUCT_ID + "=?", new String[]{product.getProductId()} );
        int count = cursor.getCount();
        cursor.close();
        if (count > 0 ){
            return true;
    ...
    public void updateProductInCart( Product product, int quantity) {
        ...
        String query = "UPDATE "+ TABLE_CART + " SET " + KEY_CART_QUANTITY + "=" + KEY_CART_QUANTITY + " + " + quantity + " WHERE " + KEY_CART_PRODUCT_ID + "=" +product.getProductId();
        SQLiteDatabase db = getWritableDatabase();
        db.execSQL(query);
    }
